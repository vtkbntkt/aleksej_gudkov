...
public class DataBase {
    ...
    public static ArrayList<TaskCardCreator> getCard(String status) {
        ...
        for (int i = 0; i < sDataBase.length; i++) {
            ...
            if (sDataBase[i][2].equalsIgnoreCase(status)) {
                likeCount = sDataBase[i][7];
                targetCompanyType = sDataBase[i][3];
                accidentAddress = sDataBase[i][4];
                regDate = sDataBase[i][5];
                daysRest = sDataBase[i][6];
                likeId = sIcLikeId;
                symbolId = getSimbolId(sDataBase[i][3]);
                itemId = Long.parseLong(sDataBase[i][0]);
    ...
    public static String getTitle(long cardId){
        ...
        return sDataBase[findItem(cardId)][1];
    }
    ...
    public static String[] getRef(long cardId){
        ...
        output[0] = sDataBase[findItem(cardId)][12];
        output[1] = sDataBase[findItem(cardId)][13];
        output[2] = sDataBase[findItem(cardId)][14];
